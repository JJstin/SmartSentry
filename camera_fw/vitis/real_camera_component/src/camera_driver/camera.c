...
u8 cameraWrite(u16 address, u8 value){
    u8 threeByte[3];
    threeByte[0] = (address >> 8) & 0xFF;
    threeByte[1] = (address) & 0xFF;
    threeByte[2] = value;
    return XIic_Send(i2cInst.BaseAddress, CAMERA_I2C_ADDR, threeByte,3,XIIC_STOP);
}
...
void camConfigureSensor(void) {
  ...
  usleep(1000);
//   while(1) {
//     xil_printf("About to set 0x3013 again...\n\r");
//     // cameraWrite(0x3013, 0x93);
//     // usleep(1000);
//     cameraVerify(0x3013, 0x93);
//     usleep(1000000);
//   }
...
u8 capture(u8 * ImgBuffer, int * bytes){
    ...
    arducamWriteFIFOControlReg(FIFO_CLEAR_CAPTURE_DONE_FLAG);
    arducamWriteFIFOControlReg(FIFO_START_CAPTURE);
