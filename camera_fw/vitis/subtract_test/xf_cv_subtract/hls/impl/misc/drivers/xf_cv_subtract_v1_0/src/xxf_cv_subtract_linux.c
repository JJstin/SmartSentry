...
typedef struct {
    ...
    XXf_cv_subtract_uio_map maps[ MAX_UIO_MAPS ];
} XXf_cv_subtract_uio_info;

/***************** Variable Definitions **************************************/
static XXf_cv_subtract_uio_info uio_info;
...
static int uio_info_read_name(XXf_cv_subtract_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XXf_cv_subtract_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XXf_cv_subtract_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XXf_cv_subtract_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XXf_cv_subtract_Initialize(XXf_cv_subtract *InstancePtr, const char* InstanceName) {
	XXf_cv_subtract_uio_info *InfoPtr = &uio_info;
